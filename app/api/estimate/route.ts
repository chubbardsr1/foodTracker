  ...
  try {
    ...
    if (response.status === 401 || response.status === 403) {
      return Response.json(
        {
          error:
            "The meal assistant could not authenticate with Google. Check the server configuration.",
        },
        { status: 502 },
      )
    }
    ...
    if (!text.trim()) {
      return Response.json(
        {
          error:
            "The assistant returned an empty answer. Try again, or enter the nutrition yourself.",
        },
        { status: 502 },
      )
    }
